...
std::unique_ptr<ResultBase> ModelSSD::postprocessSingleOutput(InferenceResult& infResult) {
    ...
    slog::info << "Yuxincui detectionsNum: " << detectionsNum << slog::endl;
    ...
    for (size_t i = 0; i < detectionsNum; i++) {
        ...
        slog::info << "Yuxincui confidence: " << confidence << slog::endl;
        /** Filtering out objects with confidence < confidence_threshold probability **/
        if (confidence > confidenceThreshold) {
            DetectedObject desc;
            slog::info << "Yuxincui xmin: " << detections[i * objectSize + 3] << slog::endl;
            slog::info << "Yuxincui ymin: " << detections[i * objectSize + 4] << slog::endl;
            slog::info << "Yuxincui xmax: " << detections[i * objectSize + 5] << slog::endl;
            slog::info << "Yuxincui ymax: " << detections[i * objectSize + 6] << slog::endl;
            ...
            desc.height = clamp(detections[i * objectSize + 6] * internalData.inputImgHeight,
                                ...
                                static_cast<float>(internalData.inputImgHeight)) -
                          desc.y;
            slog::info << "Yuxincui x: " << desc.x << slog::endl;
            slog::info << "Yuxincui y: " << desc.y << slog::endl;
            slog::info << "Yuxincui width: " << desc.width << slog::endl;
            slog::info << "Yuxincui height: " << desc.height << slog::endl;
...
void ModelSSD::prepareSingleOutput(std::shared_ptr<ov::Model>& model) {
    ...
    slog::info << "Yuxincui detectionsNumId: " << detectionsNumId << slog::endl;
    objectSize = shape[ov::layout::width_idx(layout)];
    slog::info << "Yuxincui objectSize: " << objectSize << slog::endl;
    if (objectSize != 7) {
