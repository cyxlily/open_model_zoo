...
public:
    ...
    VideosCapture(bool loop) : loop{loop} {}
    virtual double fps() const = 0;
    virtual cv::Mat read() = 0;
    virtual std::vector<cv::Mat> get_batch() = 0;
    virtual std::vector<cv::Mat> get_batch_fake() = 0;
    virtual std::string getType() const = 0;
    const PerformanceMetrics& getMetrics() {
        return readerMetrics;
    }
    virtual ~VideosCapture() = default;
...
std::unique_ptr<VideosCapture> openVideosCapture(
